...
**NotebookLM は 2026年7月16日に「Gemini Notebook」へ改称されました。** アドレスも `notebook.google.com` に変わりましたが、旧アドレスは自動転送されるので、既存のリンクは切れません。

**これから書くものは `Gemini Notebook（旧NotebookLM）` と表記します。** 初出で一度そう書けば、以降は `Gemini Notebook` だけで構いません。まだ画面に旧名が表示されている人がいるので、旧名を併記して混乱を防ぎます。

サイト内には旧名のままの記述が130か所ほど残っていますが、**過去の記事はさかのぼって直しません**。Googleが自動転送しているため実害が小さいのと、第2回セミナー資料「NotebookLMで子供の学びを変える」のように、**実物のタイトルまで書き換えると中身と食い違う**ものが混ざっているためです。

検索対策として、`tags` には `Gemini Notebook` と `NotebookLM` の**両方**を入れてください。会員は旧名で検索します。
